...
class Usuario extends Model{
    ...
    public function getTotalTweets(){
        $query = "select count(*) form tweets where id_usuario = ?";
        $stmt= $this->db->prepare($query);
        $stmt->bindValue(1,$this->__get('id',$this->__get('id')));
        $stmt->execute();
        return $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    public function getTotalSeguindo(){
        $query = "select count(*) form usuario_seguidores where id_usuario = ?";
        $stmt= $this->db->prepare($query);
        $stmt->bindValue(1,$this->__get('id',$this->__get('id')));
        $stmt->execute();
        return $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    public function getTotalSeguidores(){
        $query = "select count(*) form usuario_seguidores where id_usuario_seguindo = ?";
        $stmt= $this->db->prepare($query);
        $stmt->bindValue(1,$this->__get('id',$this->__get('id')));
        $stmt->execute();
        return $stmt->fetch(\PDO::FETCH_ASSOC);
